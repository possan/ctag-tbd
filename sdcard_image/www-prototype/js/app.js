...
(function() {
  ...
  function setupTheme() {
    var saved = localStorage.getItem('tbd-theme') || 'dark';
    applyTheme(saved);

    var toggle = document.getElementById('theme-toggle');
    if (toggle) {
      toggle.addEventListener('click', function() {
        var current = document.documentElement.classList.contains('sl-theme-dark') ? 'dark' : 'light';
  ...
  function applyTheme(theme) {
    ...
    var icon = document.querySelector('#theme-toggle sl-icon');
    if (icon) icon.setAttribute('name', theme === 'dark' ? 'sun' : 'moon');
  }
    ...
    function check() {
      fetch('/api/v1/health', { method: 'GET' })
        .then(function() {
          ...
          var txt = document.getElementById('conn-pill-text');
