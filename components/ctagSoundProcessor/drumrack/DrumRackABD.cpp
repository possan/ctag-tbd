...
void DrumRackABD::Init(const DrumRackInitData *initdata) {
    ...
    initdata->rack->registerParamAndCC(initdata, "accent", 7, [&](const int val){ accent = val;});
    ...
    initdata->rack->registerParamAndCC(initdata, "a_fm", 10, [&](const int val){ a_fm = val;});
    initdata->rack->registerParamAndCC(initdata, "s_fm", 11, [&](const int val){ s_fm = val;});
