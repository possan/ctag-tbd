...
inline int ctagSoundProcessorVctrSnt::process_param_trig(const ProcessData &data, int trig_myparm, int my_parm,
...
void ctagSoundProcessorVctrSnt::prepareWavetable(const int16_t **wavetables, int currentBank, bool *isWaveTableGood,
    ...
    sample_rom.ReadSlice(&((*ibuffer)[bufferOffset]), bankOffset, 0, 256*64);
...
void ctagSoundProcessorVctrSnt::Process(const ProcessData &data) {
    ...
    f_MasterPitch += f_MasterTune *
                     12;                         // Please note: that this tuning in combination with quantize can be used for error-correction of CV, too!

    float f_MasterPitch_CV = f_MasterPitch;
    if (cv_MasterPitch !=
        -1)                                 // Please note: we save the CV-assciated pitch here seperately, this may be excluded per Oscillator-Group via an option!
        f_MasterPitch_CV = f_MasterPitch + data.cv[cv_MasterPitch] *
    ...
    float f_MasterPitch_SubOSC = t_ExclSubOSCmasterPitch ? f_MasterPitch
                                                         : f_MasterPitch_CV;   // We either add the CV-masterpitch for groups of oscillators or ignore it!
    ...
    float f_PitchSubOsc_A = PitchSubOsc_A / 100.f +
                            60.f;                    // Range is -3600...3600 as "MIDI notes", relative to middle C
    if (cv_PitchSubOsc_A != -1)
        f_PitchSubOsc_A += data.cv[cv_PitchSubOsc_A] *
                           60.f;          // We expect "MIDI"-notes 0-59 for 5 octaves (5*12) which fit into 0...+5V with 1V/Oct logic!
    ...
    if (t_FreqModActive) {
        ...
        {
            f_FreqModAmnt += 3.5f * lfoRandom.Process() *
                             f_FreqModAnalog;    // Within range randomly modify frequency modulation amount
            CONSTRAIN(f_FreqModAmnt, 0.f, 7.f);
            f_FreqModSpeed += 10.f * lfoRandom.Process() *
                              f_FreqModAnalog;   // Within range randomly modify frequency modulation speed
            CONSTRAIN(f_FreqModSpeed, 0.01f, 20.f);
        }
        lfoPitch.SetFrequency(f_FreqModSpeed);
        float pitchVal = lfoPitch.Process();
        pitchVal = morph_sine_wave(pitchVal, i_FreqModType,
                                   e_PitchMod);    // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
        f_PitchMod = pitchVal *
                     f_FreqModAmnt;    // We will add this to each Oscillator, the value is either 0 (if inactive) or the current offset.
    ...
    const float f_freq_A =
            plaits::NoteToFrequency(60 + f_tune_A * 12.f + f_pitch_A + f_MasterPitch_WT + f_PitchMod_WT) * 0.998f;
    ...
    if (t_FilterLFOon_A) {
        ...
        f_filterLFO_A = morph_sine_wave(f_filterLFO_A, i_LFOfilterType_A,
                                        e_filterLFO_A);    // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
    ...
    if (t_ScanWavTbl_A) {
        f_LFO_WT_A = morph_sine_wave(f_LFO_WT_A, i_LFOzScanType_A,
                                     e_WT_A);   // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
    ...
    if (isWaveTableGood_A) {
        ...
        if (t_ModulateSubOscXfade_A && t_LfoWTxFadeActive_2)   // Modulate SubOSC a Xfade with LFO 2 of WT XFade?
        {
            ...
            lfo_Xfade_SubOSC_A = morph_sine_wave(lfo_Xfade_SubOSC_A, i_LfoTypeWTxFade_2,
                                                 e_XfadeWT_2);   // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
        ...
        for (int i = 0; i < bufSz; i++) {
            if (t_SubOscPWM_A)   // PWM modulated square-wave as sub-oscillator?
            {
                ...
                subOscVal += f_LFO_pwm *
                             f_PWMintensity; // Add "PWM-offset", we take the amount and LFO-speed for our sub-oscillator from the settings of the table-Z-scan!
            ...
            out_A[i] = out_A[i] * (1.f - f_SubOscFade_A) * lfo_Xfade_SubOSC_A + subOscVal * f_VolWT_A * f_SubOscFade_A *
                                                                                lfo_Xfade_SubOSC_A;    // Crossfade the Wavetable and its suboscillator
        }
    ...
    const float f_freq_C =
            plaits::NoteToFrequency(60 + f_tune_C * 12.f + f_pitch_C + f_MasterPitch_WT + f_PitchMod_WT) * 0.998f;
    ...
    if (t_FilterLFOon_C) {
        ...
        f_filterLFO_C = morph_sine_wave(f_filterLFO_C, i_LFOfilterType_C,
                                        e_filterLFO_C);    // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
    ...
    if (isWaveTableGood_C) {
        ...
        if (t_ModulateSubOscXfade_C && t_LfoSAMPxFadeActive_2)   // Modulate SubOSC C Xfade with LFO 2 of Sample XFade?
        {
            ...
            lfo_Xfade_SubOSC_C = morph_sine_wave(lfo_Xfade_SubOSC_C, i_LfoTypeSAMPxFade_2,
                                                 e_XfadeSAMP_2);   // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
        ...
        for (int i = 0; i < bufSz; i++) {
            ...
            out_C[i] = out_C[i] * (1.f - f_SubOscFade_C) * lfo_Xfade_SubOSC_C + subOscVal * f_SubOscFade_C *
                                                                                lfo_Xfade_SubOSC_C;    // Crossfade the Wavetable and its suboscillator
        }
    ...
    float panValue = (fastsin(lfoPanner.Process()) + 1.f) * M_PI /
                     4.f;             // Panning based on the algorithm found here: https://audioordeal.co.uk/how-to-build-a-vst-lesson-2-autopanner/
    ...
    if (t_LfoWTxFadeActive_1) {
        ...
        f_lfo_value_Xfade_WT_1 = morph_sine_wave(f_lfo_value_Xfade_WT_1, i_LfoTypeWTxFade_1,
                                                 e_XfadeWT_1);   // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
    ...
    if (t_LfoWTxFadeActive_2 && !t_ModulateSubOscXfade_C)   // Modulate SubOSC Xfade instead?
    {
        ...
        f_lfo_value_Xfade_WT_2 = morph_sine_wave(f_lfo_value_Xfade_WT_2, i_LfoTypeWTxFade_2,
                                                 e_XfadeWT_2);   // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
    ...
    if (t_LfoSAMPxFadeActive_1) {
        ...
        f_lfo_value_Xfade_SMP_1 = morph_sine_wave(f_lfo_value_Xfade_SMP_1, i_LfoTypeSAMPxFade_1,
                                                  e_XfadeSAMP_1);   // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
    ...
    if (t_LfoSAMPxFadeActive_2 && !t_ModulateSubOscXfade_C)   // Modulate SubOSC Xfade instead?
    {
        ...
        f_lfo_value_Xfade_SMP_2 = morph_sine_wave(f_lfo_value_Xfade_SMP_2, i_LfoTypeSAMPxFade_2,
                                                  e_XfadeSAMP_2);   // Convert sine-wave to square-wave, pseudo-triangle, S&H and so on if desired
    ...
    for (uint32_t i = 0; i < bufSz; i++) {
        ...
        if (t_StereoSplit) {
            f_val_result_l =
                    out_A[i] * f_XfadeWaveTbls_A;        // Get precalculated data from wavetable-voice A for output
            f_val_result_r =
                    out_C[i] * f_XfadeWaveTbls_C;       // Mix with precalculated data from wavetable-voice B for output
            ...
            if (t_PannerOn && f_PanAmnt_HI != 0.f)            // Autopanner is on / has amount > 0
            {
                ...
                f_val_result_l = f_val_result_l_dry * f_PanAmnt_LO + f_val_result_l *
                                                                     f_PanAmnt_HI;  // Crossfade with original signal, depending on needed amount of panner
                CONSTRAIN(f_val_result_l, -1.f, 1.f);         // Limit result to max. audio-level
                data.buf[i *
                         2] = f_val_result_l;         // Left channel (sound in principle is mono, without the right channel we will have a tremelo instead of an auto-panner effect!

                f_val_result_r = f_val_result_r_dry * f_PanAmnt_LO + f_val_result_r *
                                                                     f_PanAmnt_HI;  // Crossfade with original signal, depending on needed amount of panner
        ...
        } else      // XFades are not panned Left/Right, but we still may have an auto-panner effect on the mono-signal
        {
            ...
            if (t_PannerOn && f_PanAmnt_HI != 0.f)       // Autopanner is on / has amount > 0
            {
                ...
                f_val_result_l = f_val_result * f_PanAmnt_LO + f_val_result_l *
                                                               f_PanAmnt_HI;  // Crossfade with original signal, depending on needed amount of panner
                CONSTRAIN(f_val_result_l, -1.f, 1.f);         // Limit result to max. audio-level
                data.buf[i *
                         2] = f_val_result_l;         // Left channel (sound in principle is mono, without the right channel we will have a tremelo instead of an auto-panner effect!

                f_val_result_r = f_val_result * f_PanAmnt_LO + f_val_result_r *
                                                               f_PanAmnt_HI;  // Crossfade with original signal, depending on needed amount of panner
                CONSTRAIN(f_val_result_r, -1.f, 1.f);         // Limit result to max. audio-level
                data.buf[i * 2 + 1] = f_val_result_r;       // Right channel output
...
void ctagSoundProcessorVctrSnt::Init(std::size_t blockSize, void *blockPtr) {
    ...
    blockPtr = static_cast<uint8_t *>(blockPtr) + 512 * 4;
    ...
    oscSub_A.SetSampleRate(
            44100.f);      // MB ### 20210223 changed from: (44100.f / bufSz); because Process() is called in main loop!
    oscSub_A.SetFrequency(1.f);
    oscSub_C.SetSampleRate(
            44100.f);      // MB ### 20210223 changed from: (44100.f / bufSz); because Process() is called in main loop!
    oscSub_C.SetFrequency(1.f);

    // --- LFO to modulate the pitch of the Oscillators ---
    lfoPitch.SetSampleRate(44100.f /
