...
void RackClap::Init(const PickSeqRackInitData *initdata) {
    ...
    initdata->rack->registerMacroParamAndCC(initdata, "scale", 11, [&](const int val){ scale = val;});
    initdata->rack->registerMacroParamAndCC(initdata, "transient", 12, [&](const int val){ transient = val;});
